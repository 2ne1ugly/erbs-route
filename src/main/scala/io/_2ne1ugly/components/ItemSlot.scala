...
import io._2ne1ugly.erbs.data.{EquipType, Item, ItemRarity}
import org.scalajs.dom.document

object ItemSlot {

  def apply(item: Either[EquipType, Item]) = {
    ...
    div(
      ...
      item.fold(renderPlaceHolder, renderImg),
      item.fold(_ => emptyMod, renderTooltip(_, showTooltipVar, mousePositionVar))
    )
  }

  def renderPlaceHolder(equipType: EquipType) =
    div(equipType.getClass.getSimpleName)
  ...
  def renderTooltip(item: Item, showTooltipVar: Var[Boolean], mousePositionVar: Var[(Double, Double)]) =
    div(
